...
import com.hyve.ui.core.domain.elements.isAutoGenerated
...
class UIExporter(
    ...
    fun export(document: UIDocument): Result<String, ExportError> {
        return try {
            ...
            // Group #Root is only unwrapped when it has children; a childless
            // Group #Root is a valid authored element and must be preserved.
            val isSyntheticRoot = document.root.type.value == "Root"
            val isEditorRoot = document.root.type.value == "Group" &&
                document.root.id?.value == "Root" &&
                document.root.children.isNotEmpty()
            if (isSyntheticRoot || isEditorRoot) {
    ...
    private fun exportElement(element: UIElement, formatter: Formatter) {
        ...
        val opening = formatter.formatElementOpening(
            exportType,
            exportableId(element.id)?.let { "#$it" }
        )
        formatter.appendLine(opening)

        // Properties
        formatter.indented {
            exportProperties(exportProperties, formatter, element.children)
    ...
    private fun exportIdOnlyBlock(element: UIElement, formatter: Formatter) {
        ...
        formatter.indented {
            exportProperties(element.properties, formatter, element.children)
    ...
    private fun exportProperties(
        ...
        formatter: Formatter,
        siblingChildren: List<UIElement> = emptyList()
    ) {
        properties.entries().forEach { (name, value) ->
            ...
            if (name.value.startsWith("@")) {
                exportScopedStyle(name.value, value, formatter, siblingChildren)
                return@forEach
            }
    ...
    /**
     * Export an element-scoped style definition, preserving the original format.
     *
     * Handles:
     * - Type constructor: @Style = TypeName(Props...);
     * - Element-based: @Style = TypeName { Props...; Children... };
     * - Tuple: @Style = (Props...);
     * - Simple value: @Style = value;
     */
    private fun exportScopedStyle(
        styleName: String,
        value: PropertyValue,
        formatter: Formatter,
        siblingChildren: List<UIElement>
    ) {
        if (value !is PropertyValue.Tuple) {
            // Non-tuple value — export as-is
            formatter.appendLine("$styleName = ${formatPropertyValue(value)};")
            return
        }

        val entries = value.values
        val typeName = (entries["_typeName"] as? PropertyValue.Text)?.value
        val elementType = (entries["_elementType"] as? PropertyValue.Text)?.value

        // Filter out metadata keys (keep _spread_, _value, _styleAlias)
        val cleanEntries = entries.filterKeys { key ->
            key != "_typeName" && key != "_elementType"
        }

        when {
            // Element-based style: @Style = TypeName { Props...; Children };
            elementType != null -> {
                formatter.appendLine("$styleName = $elementType {")
                formatter.indented {
                    // Export properties
                    cleanEntries.forEach { (key, propValue) ->
                        if (key.startsWith("_spread_")) {
                            formatter.appendLine("${formatPropertyValue(propValue)};")
                        } else if (key == "_value") {
                            // skip — this is a metadata key
                        } else {
                            formatter.appendLine("$key: ${formatPropertyValue(propValue)};")
                        }
                    }

                    // Find and export children from the _ScopedStyleChildren wrapper
                    val childWrapper = siblingChildren.firstOrNull { child ->
                        child.type.value == "_ScopedStyleChildren" &&
                            (child.properties[PropertyName("_forStyle")] as? PropertyValue.Text)?.value == styleName
                    }
                    if (childWrapper != null) {
                        if (cleanEntries.isNotEmpty() && childWrapper.children.isNotEmpty() && config.addBlankLineBetweenElements) {
                            formatter.appendBlankLine()
                        }
                        exportChildren(childWrapper.children, formatter)
                    }
                }
                formatter.appendLine("};")
            }
            ...
            // Type constructor: @Style = TypeName(Props...);
            typeName != null -> {
                val propsStr = cleanEntries.entries.joinToString(", ") { (key, propValue) ->
                    if (key.startsWith("_spread_")) {
                        formatPropertyValue(propValue)
                    } else {
                        "$key: ${formatPropertyValue(propValue)}"
                    }
                }
                formatter.appendLine("$styleName = $typeName($propsStr);")
            }

            // Tuple or simple value — use existing logic
            else -> {
                val styleValue = unwrapStyleValue(value)
                formatter.appendLine("$styleName = $styleValue;")
            }
        }
    }
    ...
    private fun exportChildren(children: List<UIElement>, formatter: Formatter) {
        children.forEachIndexed { index, child ->
            // Skip synthetic wrappers for element-scoped style children
            if (child.type.value == "_ScopedStyleChildren") {
    ...
    /**
     * Returns the ID string for export, or null if the ID is auto-generated.
     * Auto-generated IDs (assigned to ID-less elements for delta tracking)
     * are stripped from exported .ui files to keep them clean.
     */
    private fun exportableId(id: ElementId?): String? {
        if (id == null || id.isAutoGenerated()) return null
        return id.value
    }
}
