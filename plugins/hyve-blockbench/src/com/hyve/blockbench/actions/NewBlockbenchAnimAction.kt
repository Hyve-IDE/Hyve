...
/**
 * Action to create a new Blockbench animation file (.blockyanim).
 */
class NewBlockbenchAnimAction : HyveProjectAction(
    "Blockbench Animation",
    "Create a new Blockbench animation file",
    null
) {
    override fun performAction(project: Project, e: AnActionEvent) {
        ...
        val fileName = Messages.showInputDialog(
            project,
            "Enter the name for the new animation file:",
            "New Blockbench Animation",
            null,
            "new_animation",
            null
        ) ?: return

        val fullFileName = if (fileName.endsWith(".blockyanim")) fileName else "$fileName.blockyanim"

        val created = runWriteAction {
            try {
                val file = directory.createChildData(this, fullFileName)
                file.setBinaryContent(DEFAULT_ANIM_TEMPLATE.toByteArray())
                file
            } catch (ex: Exception) {
    ...
    companion object {
        private val DEFAULT_ANIM_TEMPLATE = """
            {
              "meta": {
                "format_version": "4.10",
                "model_format": "free"
